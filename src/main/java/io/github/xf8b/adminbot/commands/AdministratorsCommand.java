...
public class AdministratorsCommand extends AbstractCommand {
    ...
    public Mono<Void> onCommandFired(@NotNull CommandFiredEvent event) {
        ...
        MongoCollection<Document> mongoCollection = adminBot.getMongoDatabase()
                .getCollection("administratorRoles");
        boolean isAdministrator = PermissionUtil.canMemberUseCommand(adminBot, guild, member, this);
        switch (action) {
            case "add", "addrole" -> {
                if (isAdministrator) {
                    if (event.getValueOfFlag(ROLE).isEmpty() || event.getValueOfFlag(ADMINISTRATOR_LEVEL).isEmpty()) {
                        return channel.createMessage("Huh? Could you repeat that? The usage of this command is: `" + this.getUsageWithPrefix(adminBot, guildId) + "`.").then();
                    }
